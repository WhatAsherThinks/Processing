...
float x;
float y;
float easing = 0.02; //this represents the delay of the birds
...
ellipse(400,75,300,300);
  ...
  float dx = targetX - x; //This shows the distanse between birds and mouse on x
  x += dx * easing; //Thisshows the delay in the birds following mouse
  
  float targetY = mouseY;
  float dy = targetY - y; //This shows the distance between birds and mouse on y
  y += dy * easing; //This shows the delay between the mouse
  imageMode(CENTER);
  image(tweetpic, mouseX, mouseY);   //picture of bird; follows the mouse along the x-axis, width and heighe/2  
}
